...
def drawOrder(data, outputDir, offset):
    ...
    # Create a scatter plot with different colors for different values
    ax = axes
    cs = ax.scatter(V1DF["x_mds"], V1DF["y_mds"], c=V1Tuning, alpha=0.6, cmap=custom_colormap, marker="o", s=10)
    ax.scatter(VnDF["x_mds"], VnDF["y_mds"], c=VnTuning, alpha=1, cmap=custom_colormap, marker="v", s=10)
    ax.scatter(VnDF["x_mds"].iloc[offset-V1Count], VnDF["y_mds"].iloc[offset-V1Count], color="black", alpha=1, marker="v", s=10)
...
def create_video_animation(
    ...
    """
    Create a simple HTML animation showing node activation/order.
    Uses saved batch_info and pred_colors_array from simulation (no re-simulation or re-computation).
    """
...
def create_timestamp_plot(
    ...
    - V1 nodes: dark purple
    - Vn nodes: each batch gets a distinct hue (HSV rainbow), within-batch
      nodes vary in saturation to show processing order (fp/pf/random).
    - White quadratic spline V1-V2 boundary line (KNN k=4 detection).
    - Vertical colorbar (short & wide) and 'Timestamp' title.
    """
    import colorsys
