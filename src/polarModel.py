...
class VisualMatrix3D(object):
    def __init__(self, dataDF, param, outputDir):
        ...
        # Incremental propagation cache: avoids recomputing C @ W @ D every step.
        ...
        # Pre-compute V1 tuning tensor (avoids repeated DF access in inner loop)
    ...
    def initMatrix(self, DF, radius=6.0, tangent=30.0, mode="sphere", distance_mode="polar"):
        ...
        # Load x, y, z coordinates (original 3D coordinates)
        ...
        x1 = x_coords.unsqueeze(1)  # (N, 1)
        x2 = x_coords.unsqueeze(0)  # (1, N)
        ...
        if distance_mode == "sphere_geo":
            # --- TRUE geodesic distance on the R=100 sphere, decomposed in each
            #     SOURCE node's LOCAL tangent plane (Riemannian log map) into radial
            #     (away from the fovea / is_center) and tangential components. ---
            # For source i (unit u_i) and target j (unit u_j):
            #   g_ij     = R * arccos(u_i . u_j)                  geodesic distance ON the sphere
            #   log_i(j) = g_ij * normalize(u_j - (u_i.u_j) u_i)  tangent vec at i, |log_i(j)|=g_ij
            #   e_r(i)   = normalize( (u_i.u_c) u_i - u_c )        away-from-fovea, in i's tan-plane
            #   e_t(i)   = u_i x e_r(i)                            perpendicular, in i's tan-plane
            ...
            sz = torch.tensor(DF["sz"].values, device=self.device, dtype=torch.float64)
            P = torch.stack([sx, sy, sz], dim=1)
            U = P / torch.linalg.norm(P, dim=1, keepdim=True)            # unit vecs (N,3)

            center_flags = torch.tensor(DF["is_center"].values, device=self.device, dtype=torch.long)
            center_idx = int(torch.where(center_flags == 1)[0][0].item())
            uc = U[center_idx]                                          # fovea unit vector

            # per-source local frame in i's tangent plane (sign is irrelevant: d_r,d_t squared)
            dotc = (U @ uc).unsqueeze(1)                               # (N,1) = u_i . u_c
            er = U * dotc - uc.unsqueeze(0)                            # away-from-fovea (N,3)
            er_n = torch.linalg.norm(er, dim=1, keepdim=True)
            er = er / torch.clamp(er_n, min=1e-12)                    # normalize (center row -> ~0)
            deg = (er_n.squeeze(1) < 1e-9)                            # true only at the center node
            if deg.any():
                # radial direction is undefined at the pole; pick any UNIT tangent vector there
                ax = torch.zeros_like(U)
                jj = torch.argmin(torch.abs(U), dim=1)
                ax[torch.arange(len(U), device=self.device), jj] = 1.0
                alt = ax - (ax * U).sum(1, keepdim=True) * U
                alt = alt / torch.clamp(torch.linalg.norm(alt, dim=1, keepdim=True), min=1e-12)
                er = torch.where(deg.unsqueeze(1), alt, er)           # overwrite AFTER normalizing
            et = torch.linalg.cross(U, er)                             # perpendicular (N,3)
            et = et / torch.clamp(torch.linalg.norm(et, dim=1, keepdim=True), min=1e-12)

            # pairwise geodesic magnitude / sin(theta), and u_j projected on e_r(i), e_t(i)
            cos_ij = torch.clamp(U @ U.t(), -1.0, 1.0)                 # (N,N)
            theta = torch.arccos(cos_ij)                              # (N,N) angle
            sin_ij = torch.clamp(torch.sqrt(torch.clamp(1.0 - cos_ij ** 2, min=0.0)), min=1e-9)
            scale = (R_SPHERE * theta) / sin_ij                       # (N,N) g_ij / |u_j-cos u_i|
            del cos_ij, theta
            d_r = scale * (er @ U.t())                                # (N,N) radial component
            ...
            if _os.environ.get("SPHERE_GEO_NORMALIZE", "0") == "1":
                ...
                max_mds = torch.cdist(mds_xy, mds_xy).max()
                max_sph = torch.sqrt(d_r ** 2 + d_t ** 2).max()
                nscale = (max_mds / max_sph).to(torch.float64)
                d_r = d_r * nscale
                d_t = d_t * nscale
                print(f"[sphere_geo] log-map, MDS-scale norm ON: max_geo={max_sph.item():.4f} "
                      f"max_mds={max_mds.item():.4f} scale={nscale.item():.6f}")
            else:
                print("[sphere_geo] log-map (tangent-plane geodesic decomposition), "
                      f"MDS-scale norm OFF (raw geodesic mm on R={R_SPHERE:.0f} sphere)")

            d = torch.sqrt((d_r / radius) ** 2 + (d_t / tangent) ** 2)
            kernel = torch.exp(-d).to(torch.float32)
        elif distance_mode == "euclidean":
            # --- Euclidean distance kernel ---
            kernel = torch.exp(-euclidean_distance / radius)
        elif distance_mode == "arc":
            # --- Arc distance kernel (arc mode: arc + radius) ---
            ...
            # --- Rotated elliptical kernel (polar mode) ---
    ...
    def _assign_single_vn(self, best_col, temp, resource, V1Count, sampleMode, target_degree):
        ...
        # Incremental cache update: avoids full C @ W @ D recompute next step
        ...
        # Compute predicted tuning color
    ...
    def step_iter(self, V1Count, sampleMode, batch_idx, total_batches):
        # Use cached propagation (updated incrementally in _assign_single_vn)
        ...
        # --- Tie-breaking path (bs=1, no dynamic schedule) ---
        # Find ALL Vn with the same max score and assign them simultaneously
        # using the same temp matrix (no recomputation between tied nodes).
        ...
        # --- Dynamic schedule / bs>1 path (existing logic) ---
        ...
        with tqdm(total=total_vn, desc="custom_assign") as pbar:
            for batch_id in sorted(self.custom_batch_assignments.keys()):
                ...
                for df_idx in batch_nodes_list:
                    # df_idx is the DF index; convert to Vn column index
                    # In the DF, V1 nodes come first (indices 0..V1Count-1),
                    # Vn nodes follow (indices V1Count..V1Count+VnCount-1)
                    ...
                    # --- Compute scores (same as step_iter) ---
                    indirect_propagation = self.matrixC @ self.matrixW @ self.matrixD
                    propagation = indirect_propagation[:V1Count, :]
                    resource = self.computeResource()
                    temp = torch.multiply(propagation, resource)
                    temp = temp @ self.mask

                    # Get scores for this specific Vn column
                    ...
                    # Select V1 parents (same logic as step_iter)
    ...
    def sample(self, matrix, mode):
        if mode < 0:
            # Deterministic: find argmax
            flat_idx = torch.argmax(matrix)
            return torch.unravel_index(flat_idx, matrix.shape)
        else:
            # Stochastic: sample based on weights
            weights = matrix.flatten()
            weights = weights / torch.sum(weights)
            # Convert to CPU for numpy random choice, then back to GPU indices
