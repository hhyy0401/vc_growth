...
def get_v2_v4_range(deg):
    """Find angular range covering V2-V4 nodes by identifying the largest gap (V1)."""
    if len(deg) == 0:
        return 0, 0, 0
...
def compute_dynamic_batch_sizes(DF, output_dir=None, data_name="data", tag_name="tag"):
    """Compute batch size schedule by sweeping outward from V1 edges into the V2-V4 block."""
    vn = DF[DF["area"] != 1]
    vn_t = vn["t"].values
...
def initDirectory(param, outputDir):
    """Placeholder output directory helper."""
    return outputDir
...
def plot_tuning_compare_two_panel(
    ...
    """Create and save a two-panel tuning comparison plot (true vs predicted)."""
    import os
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Output directory: plot_mirror/hier/plots/
    if distance_mode == "euclidean":
        out_base = os.path.join("../plot_mirror", "hier", "euclidean", "plots")
    else:
        out_base = os.path.join("../plot_mirror", "hier", "plots")
...
def save_baseline_results(
    ...
    """Save results and generate tuning comparison plot."""
    import os
    import numpy as np
    import pandas as pd

    output_base_dir = "../results_hierarchical"
    if distance_mode == "euclidean":
    ...
    # Two-panel plot: left=true, right=predicted (area1 uses true) - ALWAYS GENERATED
    ...
    print("Results saved (using newCode-style color calculation):")
    print(f"   Tuning: {tuning_file}")
